...
const ClickableIcon = (props:Props) => {
  return (
    <a href={props.href} className="" target={"_blank"} rel="noopener noreferrer">
      <props.Icon
        className={
          "w-5 h-5 text-gray-500 hover:text-AAsecondary transition duration-800 fill-current hover:cursor-pointer "
        }
      />
    </a>
  );
...
export default function Fotter(props:{link:string,className:string}) {
  return (
      <div className={` ${props.className} bg-AAprimary flex flex-col justify-center items-center py-8 space-y-4 `}>
        ...
        <a href={props.link} className="" target={"_blank"} rel="noopener noreferrer">
          <div
    ...
    text-sm  space-y-2  "
          >
            ...
            <span className="text-xs flex flex-row items-center space-x-2 group-hover:text-AAsecondary">
              <GithubIcon
                className={
                  "w-4 h-4 text-gray-500 fill-current group-hover:text-AAsecondary duration-800"
                }
              />
